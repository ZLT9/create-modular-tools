...
public abstract class ThrownBoomerang extends AttackableArrow {
    ...
    protected void onHitBlock(BlockHitResult result) {
        ...
        if (!toolModulesNbt.contains(TOOL_MODULE.getType().getTag(), Tag.TAG_COMPOUND)) {
            for (String key : toolModulesNbt.getAllKeys()) {
                if (ToolUtils.MoldSlotState.fromName(toolModulesNbt.getCompound(key).getString(AllTagNames.MOLD_SLOT_STATE)) == ToolUtils.MoldSlotState.FLUID) {
                    return;
                }
            }
        }
    ...
    public void addAdditionalSaveData(CompoundTag nbt) {
        ...
        nbt.put(AllTagNames.THROWN_BOOMERANG_ITEM, boomerangItem.save(new CompoundTag()));
    }

    @Override
    public void readAdditionalSaveData(CompoundTag nbt) {
        super.readAdditionalSaveData(nbt);
        if (nbt.contains(AllTagNames.THROWN_BOOMERANG_ITEM, Tag.TAG_COMPOUND)) {
            boomerangItem = ItemStack.of(nbt.getCompound(AllTagNames.THROWN_BOOMERANG_ITEM));
        }
    }
