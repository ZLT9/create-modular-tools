...
public class ToolMaterialMoldItem extends BlockItem {
    public ToolMaterialMoldItem(Block block, Properties properties) {
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> tooltipComponents, TooltipFlag isAdvanced) {
        ...
        for (ToolModuleType toolModuleType : MoldRegistry.getCompatible(((ToolMaterialMoldBlock) getBlock()).getModularTool())) {
    ...
    public boolean isFoil(ItemStack stack) {
        ...
        if (!(getBlock() instanceof ToolMaterialMoldBlock moldBlock)) {
            return false;
        }
        ...
        for (String key : toolModulesNbt.getAllKeys()) {
            ...
            if (ToolUtils.MoldSlotState.fromName(slotNbt.getString(AllTagNames.MOLD_SLOT_STATE)) == ToolUtils.MoldSlotState.SOLID && !slotNbt.getCompound("tag").getList(ItemStack.TAG_ENCH, Tag.TAG_COMPOUND).isEmpty()) {
                return true;
            }
        }
